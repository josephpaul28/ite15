...
function food_restaurant_the_breadcrumb() {
	if (!is_home()) {
		echo '<a href="';
...
function food_restaurant_font_url(){
	...
	$ptsans = _x('on', 'PT Sans font:on or off','food-restaurant');
	
	$roboto = _x('on', 'Roboto font:on or off','food-restaurant');
	
	$OpenSans = _x('on', 'Open Sans font:on or off','food-restaurant');

	$Playball = _x('on', 'Playball font:on or off','food-restaurant');

	$Raleway = _x('on', 'Raleway font:on or off','food-restaurant');

	$Montserrat = _x('on', 'Montserrat:on or off','food-restaurant');

	$roboto_cond = _x('on', 'Roboto Condensed font:on or off','food-restaurant');
	
	if('off' !== $ptsans || 'off' !==  $roboto || 'off' !== $roboto_cond || 'off' !== $OpenSans || 'off' !== $Raleway){
		$font_family = array();
...
function food_restaurant_scripts() {
	...
	wp_enqueue_style( 'food-restaurant-style', get_stylesheet_uri() );
...
function food_restaurant_ie_stylesheet(){
	wp_enqueue_style('food-restaurant-ie', get_template_directory_uri().'/css/ie.css', array('food-restaurant-ie-style'));
...
if ( ! function_exists( 'food_restaurant_credit' ) ) {
	function food_restaurant_credit(){
			echo "<a href=".esc_url(food_restaurant_CREDIT)." target='_blank'>LogicalThemes</a>";
	}
}

define('food_restaurant_CREDIT1','http://www.logicalthemes.com/premium/food-restaurant-wordpress-theme/','food-restaurant');

if ( ! function_exists( 'food_restaurant_credit1' ) ) {
	function food_restaurant_credit1(){
			echo "<a href=".esc_url(food_restaurant_CREDIT1)." target='_blank'>Restaurant WordPress Theme</a>";
	}
}
